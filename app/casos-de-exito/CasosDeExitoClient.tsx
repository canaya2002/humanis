...
const SuccessNetworkBackground = () => {
  ...
  useEffect(() => { 
      setNodes(Array.from({ length: 25 }, (_, i) => ({ 
        id: i, 
        x: Math.random() * 100, 
        y: Math.random() * 100, 
      ...
      <svg className="absolute inset-0 w-full h-full opacity-20">
        {nodes.map((node, i) => (
           <motion.circle 
             ...
             fill="#334155" 
             animate={{ 
               cx: [`${node.x}%`, `${node.x + 8}%`, `${node.x}%`], 
               cy: [`${node.y}%`, `${node.y - 8}%`, `${node.y}%`], 
               opacity: [0.2, 0.5, 0.2] 
             }} 
...
const ModernButton = ({ children, onClick }: { children: React.ReactNode; onClick?: () => void }) => (
  <button 
    onClick={onClick} 
    className="relative group overflow-hidden font-bold text-sm tracking-widest uppercase px-8 py-3 rounded-full bg-slate-900 text-white shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-300"
  >
...
const caseStudies = [
  {
    ...
    testimonial: {
      quote: "Humanis fue la pieza clave para cumplir nuestro plan de expansión. Su velocidad superó la expectativa.",
      author: "Director de Operaciones",
      role: "Cadena Retail Líder"
    },
  ...
  },
  {
    ...
    testimonial: {
      quote: "Entendieron la complejidad técnica y cultural. El equipo que armaron es el motor de nuestro producto.",
  ...
  },
  {
    ...
    testimonial: {
      quote: "No solo reclutaron, nos consultaron para mejorar la retención desde la raíz.",
...
export default function CasosDeExitoClient() {
      ...
      <main className="relative pt-0">
          ...
          <div className="container mx-auto px-6 max-w-7xl">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
              {globalStats.map((stat, idx) => (
                <div 
                  key={idx} 
                  className="text-center p-6 border border-white/10 rounded-2xl bg-white/5 backdrop-blur-sm hover:bg-white/10 transition-colors"
                >
        ...
        <div className="py-32 bg-white text-center">
          <div className="container mx-auto px-6">
            <h2 className="text-5xl md:text-6xl font-black mb-10 text-slate-900 tracking-tighter">
